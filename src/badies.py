...
class Saucer(Shooter):
    ...
    def __init__(self, stage, saucerType, ship):
        ...
        if self.largeSaucerType:
            # make larger saucer a darker green
            Shooter.__init__(self, position, heading, newPointList, stage, shooter_color=(0, 222, 0))
        else:
            # make small saucer bright green
            Shooter.__init__(self, position, heading, newPointList, stage, shooter_color=(54, 255, 54))
    ...
    def fireBullet(self):
        if self.ship is not None:
            ...
            # adding parameter 'bullet_color' to shooter.Shooter.fireBullet and passing color allows us to change
            # the saucers bullet color
            shotFired = Shooter.fireBullet(self, heading, self.bulletTtl[self.saucerType],
                                           self.bulletVelocity, bullet_color=(181, 16, 16))
            if shotFired:
                playSound("sfire")
